...
<Warning>`includeData` and `includeDataCacheKey` are mutually exclusive. Use one or the other, not both.</Warning>
``` Http
...
For webhooks that deliver a cache snapshot instead of row data, see [Webhook cache snapshot](/api-reference/webhooks/webhook-cache).
